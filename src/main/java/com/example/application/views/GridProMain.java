...
@Route("tarefas")
public class GridProMain extends Div {

    List<TarefaDTO> database;

    {
        try {
            database = new ArrayList<>(TarefaDAO.listarTarefas());
    ...
    public GridProMain(){
        ...
        Button novaTarefa = new Button("Nova tarefa");
        novaTarefa.addThemeVariants(ButtonVariant.LUMO_PRIMARY);
        ...
        verticalLayout.add(grid, novaTarefa);
        verticalLayout.setAlignSelf(FlexComponent.Alignment.END, novaTarefa);

        novaTarefa.addClickListener(event -> {
            dialogNovaTarefa();
        });
    ...
    private void setColumns(GridPro<TarefaDTO> grid) {
        ...
        grid.addEditColumn(GridProMain::getDataCreation,
                        dataCriacaoRender)
                .custom(datePicker,
    ...
    public void dialogNovaTarefa(){
        TarefaBind tarefaBind = new TarefaBind(false);
        ...
        dialogLayout.add(tarefaBind);
        dialog.add(dialogLayout);
        dialogLayout.setPadding(false);
        dialogLayout.setSpacing(false);
        dialogLayout.setAlignSelf(FlexComponent.Alignment.STRETCH);
        dialogLayout.getStyle().set("width", "800px").set("max-width","1500px");

        Button salvar = new Button("Salvar");
        salvar.addClickListener(event -> {
            try {
                tarefaBind.getBinder().writeBean(tarefaDTO);
                persist(tarefaDTO);
                dialog.close();
